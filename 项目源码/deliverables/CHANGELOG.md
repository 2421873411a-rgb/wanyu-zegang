...
## v17.9.1 · 站点发布（审查修复轮前端收口）· 2026-09-10

- 审查修复轮（v17.10.1 批次）中三处前端模板修复随正式 release 流程上线：
  theme-color meta 先于 THEME_BOOT 引导脚本（修首帧深色模式地址栏色失效）、
  og:image 改绝对 URL（修分享卡片无图）、`--text-muted` #748cac→#566e8a
  （对比度 3.26:1→4.98:1，四个浅色表面全部 ≥4.7 达 AA）。
- SW 缓存 wanyu-shell-v50→**v51**，asset_version 17.9.1；全部 302 项 verifier、
  2026 事实闸、perf 预算、浏览器烟测 ×3、模板资产 parity 13/13 通过。
- 发布环境备注：本次为本机首次完整发布链复现——外部源（source_data 458MB 等
  9/14 锁定源）从原设备同步后 verify_sources 14/14 一致；发现并修复本机
  `core.autocrlf=true` 导致数据文件字节漂移的克隆问题。
...
- `--build-only` 删除复制版 tar/venv 逻辑，生产与 upgrade drill 复用唯一 `build_release()`。
- fresh-host Nginx 恢复 `root + try_files`；HTTPS 根路径 smoke 跟随 302 并验证最终 200。
- restore drill 在 `pg_restore` 前显式建临时库，使用 `current/venv/bin/alembic heads`，成功/失败均强制清库，并核验三周期 canonical 行数。
- 自动备份拆为受测的 `scripts/backup_database.sh`；daily/weekly/monthly 都复制可独立验证的 checksum；配置 `COS_BUCKET` 后成对上传 dump/sidecar。
- v17.9.18 的“15/15 PASS / Production Hardening 完成”因证据不足及上述断链正式撤回。v17.9.19 当前仅是代码修复候选；fresh-host、生产部署、COS 异机恢复未重跑前保持 NOT FOR PRODUCTION。

## v17.9.18 · Production Hardening（历史记录；验收已撤回）· 2026-09-08

### 当时宣称的 P1×4（后续复审证伪，见 v17.9.19）
...
- **本机恢复点（当时误称“PG 自动灾备”）**：timer 与 restore 脚本已加入，但周/月 checksum、
  建库和 immutable venv 路径均存在断链；v17.9.19 修复后仍须 COS 异机恢复证据才能称灾备。
...
- deploy.sh 当时新增 --build-only + `scripts/upgrade_drill.sh`，但 build-only 是与生产不同的复制实现；
  v17.9.19 才改为复用唯一 `build_release()`。
...
P2 收尾：postgres CI job 补 timeout-minutes（与 test 对齐）；OS 矩阵文案对齐实际准入集（24.04–25.x）；登录/注册新增 per-IP 总量桶（login_ip 30/h、register_ip 10/h——封死单 IP 换邮箱撞库/批量注册绕过面），与 (ip,email) 桶并联取严。测试 76+5 → 77+5。


## v17.9.16 · Round-6 干净轮（认证/API 面零 P0/P1 + P2 收尾）· 2026-09-08

Round 6 双镜头：认证/API 回归审计结论 **干净**（33/33 对抗探针通过、五项 v17.9.15 修复实测生效、无 P0/P1）；全链一致性审计 6 条 P2 文档偏差当场修复（CONTRACT 头部滚动+§7 勾选、§2.8 对齐代码行为、--cov 注记、PID 目录过时提示、APP_ENV 死变量、词表路径基准）。

P2 收尾：stats 双端点 cycle 补 max_length=8（与 search 对齐）；删除无容量上限的死代码同步版 _cached()；为 v17.9.15 修复补三条防回退锁（__all__ 无空串/缓存 65 键清空/label>64 拒绝）。测试 73+5 → 76+5。
...
### P1
- **LIKE 转义缺陷实现修复**：v17.9.12 的 _escape_like 替换模板经多层转写损坏为"反斜杠+SOH 控制字符"——含 %/_ 字面量的关键词/专业搜索恒为空且向 SQL 参数注入控制字节；三个独立审计镜头交叉确认。改为 lambda 构造替换（chr(92)）；反斜杠入转义类当时未落地（见本节 P2 与 v17.9.14 补修），新增"字面量仍可命中"正向门禁（旧门禁只测 total==0 挡不住此类回归）。
