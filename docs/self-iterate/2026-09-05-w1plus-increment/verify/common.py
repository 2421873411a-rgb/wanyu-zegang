"""shared helpers for q* verify scripts — batch 2 (copied from batch 1)"""
import os, re, sys, glob, json

ROOT = r'E:\zcode\择岗'
SITE = os.path.join(ROOT, '网站')
SRC = os.path.join(ROOT, '项目源码')
RUN = os.path.join(ROOT, r'docs\self-iterate\2026-09-05-w1plus-increment')
...
def block_tokens(css_files):
    """scan css blocks -> {'light': {var: val}, 'dark': {...}} with comment stripping"""
    tokens = {'light': {}, 'dark': {}}
    for p in css_files:
        c = read(p)
